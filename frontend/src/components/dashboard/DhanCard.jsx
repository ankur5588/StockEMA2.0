...
export default function DhanCard({ status, reload }) {
  ...
  const save = async (e) => {
    ...
    setBusy(true);
    try {
      await api.post("/dhan/credentials", form);
  ...
  const connect = async () => {
    setBusy(true);
    try {
      await api.post("/dhan/connect");
      toast.success("Dhan connected");
      reload?.();
    } catch (err) {
      toast.error(err?.response?.data?.detail || "Dhan connect failed");
    } finally {
      setBusy(false);
    }
  };
  ...
  const disconnect = async () => {
    setBusy(true);
    try {
      await api.post("/dhan/disconnect");
  ...
  const wipe = async () => {
    if (!window.confirm("Delete saved Dhan credentials?")) return;
    try {
      await api.delete("/dhan/credentials");
      toast.success("Dhan credentials wiped");
      reload?.();
    } catch (err) {
    ...
    <Card className="bg-surface-2 border-border rounded-sm" data-testid="dhan-card">
      <CardHeader className="pb-3 flex flex-row items-start justify-between space-y-0">
        <div>
          ...
          <CardTitle className="text-lg font-medium">Dhan</CardTitle>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {isAuth && (
            <Button
              ...
              onClick={disconnect}
              disabled={busy}
              data-testid="dhan-disconnect-btn"
      ...
      <Dialog open={setupOpen} onOpenChange={setSetupOpen}>
        <DialogContent className="bg-surface-2 border-border rounded-sm sm:max-w-lg" data-testid="dhan-setup-dialog">
          <DialogHeader>
            <DialogTitle className="font-medium tracking-tight">Dhan API Access</DialogTitle>
            <DialogDescription className="text-xs text-muted-foreground">
              Generate an access token at <span className="text-white">web.dhan.co → My Profile → Access DhanHQ APIs</span>.
              Default tokens expire every 24h unless your app is registered.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={save} className="space-y-3 pt-2">
            <div className="space-y-1.5">
              <Label className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground font-semibold">
                Client ID <span className="text-loss">*</span>
              </Label>
              <Input
                required
                ...
                placeholder="10 digit Dhan client id"
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                data-testid="dhan-access-token-input"
                className="h-9 rounded-sm bg-surface-1 border-border font-mono text-xs"
              />
              <div className="text-[10px] text-muted-foreground">
                Long JWT-like string. Regenerate daily or when expired.
              </div>
            </div>
